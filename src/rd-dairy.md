...
12.20
脑裂（Split-Brain）： 分布式系统因网络分区导致集群分裂成独立子集群，各子集群独立决策并产生多个“大脑”的致命故障

- 就像人体大脑分裂成多个独立思考部分，每个都认为自己是主导者，会引发数据冲突、服务混乱甚至永久性损坏

核心表现：从“单脑”到“多脑”的失控
正常分布式系统通常通过选举产生单个主节点（如Primary、Leader或Master）统一决策

但网络分区时，各子分区因无法通信，可能各自选举主节点。如消息队列集群中原为1主2从，分区后可能出现两个主节点，都处理写请求导致数据不一致

VMware FT的Test-and-Set服务多副本案例更典型：网络分区后，两个客户端可能分别与不同服务器交互并都认为自己获锁成为主节点，形成“双主”冲突